...
console.log('[ExtraFeatures] Extra features block loaded.');

const {
  ChannelType: ExtraChannelType,
  PermissionsBitField: ExtraPermissionsBitField,
  EmbedBuilder: ExtraEmbedBuilder,
  ActionRowBuilder: ExtraActionRowBuilder,
  ButtonBuilder: ExtraButtonBuilder,
  ButtonStyle: ExtraButtonStyle,
} = require('discord.js');

const EXTRA_OWNER_ID = '1250956931447652362';

const LOAN_OFFICER_ROLE_ID = '1528804776933064905';
const BRANCH_MANAGER_ROLE_ID = '1528804643474509996';
...
function extraParseDuration(input) {
  ...
  if (!ms || ms < 5000 || ms > 30 * 86400000) return null;
  return ms;
}

function extraGetGuildConfig(guildId) {
  const cfg =
  ...
  if (!cfg?.guilds) return {};

  if (Array.isArray(cfg.guilds)) {
    return (
      cfg.guilds.find(g => g.id === guildId) ||
      cfg.guilds.find(g => g.id === '1528804420383674559') ||
      cfg.guilds[0] ||
      {}
    );
  }
  ...
  return (
    cfg.guilds[guildId] ||
    cfg.guilds['1528804420383674559'] ||
    Object.values(cfg.guilds)[0] ||
    {}
  );
}

async function extraSafeFollowUp(interaction, content) {
  try {
    ...
    console.error('[ExtraFeatures] extraSafeFollowUp failed:', error);
  }
}

function extraGiveawayEmbed(g) {
  return new ExtraEmbedBuilder()
    .setTitle(g.ended ? '🎁 Giveaway Ended' : '🎁 Giveaway')
    .setDescription(
      [
        `**Prize:** ${g.prize}`,
    ...
    .setColor(g.ended ? 0xed4245 : 0x57f287)
    .setFooter({
      text: g.ended
...
async function extraEndGiveaway(messageId) {
  const g = extraGiveaways.get(messageId);
  if (!g || g.ended) return;

  g.ended = true;
  extraGiveaways.delete(messageId);

  const pool = Array.from(g.entries);
  const winners = [];
  ...
  const disabledRow = new ExtraActionRowBuilder().addComponents(
    new ExtraButtonBuilder()
      .setCustomId('giveaway_join')
      .setLabel('Giveaway Ended')
      .setStyle(ExtraButtonStyle.Secondary)
      .setEmoji('🎉')
      .setDisabled(true)
  );
  ...
  await g.message
    .edit({
      embeds: [extraGiveawayEmbed(g)],
      components: [disabledRow],
    })
    .catch(error => {
      console.error('[ExtraFeatures] Failed to edit giveaway message:', error);
    });

  if (winners.length > 0) {
    await g.message.channel
      .send({
        content: `🎉 **Giveaway ended!** Winner(s) of **${g.prize}**: ${winners
          .map(id => `<@${id}>`)
          .join(', ')}`,
      })
      .catch(error => {
        console.error('[ExtraFeatures] Failed to announce giveaway winner:', error);
      });
  } else {
    await g.message.channel
      .send({ content: '🎉 **Giveaway ended!** No one entered.' })
      .catch(error => {
        console.error('[ExtraFeatures] Failed to announce giveaway end:', error);
      });
  }
}
...
async function extraCreateTicketFromButton(interaction, info) {
  ...
  const gc = extraGetGuildConfig(interaction.guild.id);

  await interaction.deferReply({ ephemeral: true }).catch(error => {
    console.error('[ExtraFeatures] deferReply failed:', error);
  });

  if (typeof createTicket !== 'function') {
    console.error('[ExtraFeatures] createTicket is not a function.');
    await extraSafeFollowUp(interaction, 'Ticket system not found.');
    return null;
  }
...
async function extraApplyOverwrites(interaction, ch, roleIds = []) {
  await interaction.guild.roles.fetch().catch(() => {});

  const allow = [
    ExtraPermissionsBitField.Flags.ViewChannel,
    ExtraPermissionsBitField.Flags.SendMessages,
    ExtraPermissionsBitField.Flags.ReadMessageHistory,
    ExtraPermissionsBitField.Flags.AttachFiles,
    ExtraPermissionsBitField.Flags.EmbedLinks,
  ];

  const botAllow = [
    ...allow,
    ExtraPermissionsBitField.Flags.ManageChannels,
    ExtraPermissionsBitField.Flags.ManageMessages,
  ];
  ...
  await ch.permissionOverwrites.set(overwrites).catch(error => {
    console.error('[ExtraFeatures] Failed to set ticket overwrites:', error);
  });
}

async function handleExtraFeatures(interaction) {
  if (!interaction) return false;

  // Loan ticket button
  if (
    interaction.isButton?.() &&
    interaction.customId === 'open_loan_ticket'
  ) {
    console.log('[ExtraFeatures] open_loan_ticket clicked.');
    ...
    await extraApplyOverwrites(interaction, ch, [
      LOAN_OFFICER_ROLE_ID,
      BRANCH_MANAGER_ROLE_ID,
    ]);

    await extraSafeFollowUp(interaction, `Loan ticket created: ${ch}`);

    return true;
  }

  // Private Banking ticket button
  if (
    interaction.isButton?.() &&
    interaction.customId === 'open_private_bank_ticket'
  ) {
    console.log('[ExtraFeatures] open_private_bank_ticket clicked.');
    ...
    const canOpen =
      member?.roles?.cache?.has(BRANCH_MANAGER_ROLE_ID) ||
      member?.roles?.cache?.has(PLUTUS_OWNER_ROLE_ID) ||
      member?.permissions?.has(ExtraPermissionsBitField.Flags.Administrator);

    if (!canOpen) {
      await interaction
        .reply({
          content:
            'Only Branch Managers or Plutus Owner can open this private banking ticket.',
          ephemeral: true,
        })
        .catch(() => {});
      return true;
    ...
    const category = interaction.guild.channels.cache.find(
      c =>
        c.type === ExtraChannelType.GuildCategory &&
        c.name === PRIVATE_BANK_CATEGORY_NAME
    );
    ...
    if (category) {
      await ch
        ...
        .catch(error => {
          console.error(
            '[ExtraFeatures] Failed to move private ticket to category:',
            error
          );
        });
    }
    ...
    await ch
      .setName(`private-${safeUser}-${interaction.user.id.slice(-4)}`)
      .catch(() => {});
  ...
  if (
    ...
    console.log('[ExtraFeatures] giveaway_join clicked.');

    const g = extraGiveaways.get(interaction.message.id);
    ...
    await interaction
      ...
      .catch(error => {
        console.error('[ExtraFeatures] Failed to update giveaway embed:', error);
      });
  ...
  if (
    ...
    console.log(
      '[ExtraFeatures] /giveaway command used by',
      interaction.user.id
    );

    try {
      if (interaction.user.id !== EXTRA_OWNER_ID) {
        await interaction
          .reply({
            content: 'Only the owner can create giveaways.',
            ephemeral: true,
          })
          .catch(() => {});
        return true;
      ...
      const ms = extraParseDuration(duration);

      if (!ms) {
        await interaction
          .reply({
            content:
              'Invalid duration. Use examples like `10s`, `5m`, `2h`, or `1d`.',
            ephemeral: true,
          })
          .catch(() => {});
        return true;
      ...
      await interaction.deferReply({ ephemeral: true }).catch(error => {
        console.error('[ExtraFeatures] giveaway deferReply failed:', error);
      });
      ...
      const row = new ExtraActionRowBuilder().addComponents(
        new ExtraButtonBuilder()
          .setCustomId('giveaway_join')
          .setLabel('Join Giveaway')
          .setStyle(ExtraButtonStyle.Primary)
          .setEmoji('🎉')
      );
      ...
      g.message = msg;
      extraGiveaways.set(msg.id, g);

      setTimeout(() => extraEndGiveaway(msg.id), ms);

      await extraSafeFollowUp(interaction, `Giveaway created: ${msg.url}`);
    } catch (error) {
      console.error('[ExtraFeatures] /giveaway handler failed:', error);
      await extraSafeFollowUp(
        interaction,
        'Giveaway failed: ' + (error.message || 'Unknown error')
      );
    }
...
const __originalHandleInteractionCreate =
...
module.exports.handleInteractionCreate = async function (...args) {
  ...
  try {
    ...
    console.error('[ExtraFeatures] handler error:', error);

    try {
      if (interaction && !interaction.replied && !interaction.deferred) {
        await interaction.reply({
          content: 'Internal interaction error.',
          ephemeral: true,
        });
  ...
  if (__originalHandleInteractionCreate) {
    return __originalHandleInteractionCreate.apply(this, args);
  }
};
// --- EXTRA FEATURES END ---
